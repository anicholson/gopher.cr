...
trace_level = Trace.default_level
...
OptionParser.parse! do |parser|
  ...
  {% if flag?(:trace) %}
    parser.on("-l LEVEL", "--log-level=LEVEL", "Set logging level debug|info|warn|error (default: error)") { |level| trace_level = Trace::Level.from_value(level) }
  {% end %}

  parser.on("-h", "--help", "Show this help") { puts parser; exit 0 }
end

Trace.default_level = trace_level
